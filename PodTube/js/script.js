function filtrarItens(button) {
  const categoriaSelecionada = button.getAttribute('data-categoria');
  const itens = document.querySelectorAll('.item');

  itens.forEach((item) => {
    const categoriasItem = item.classList; // Pega todas as classes do item
    item.style.display = 'none'; // Oculta todos os itens por padrão

    if (categoriasItem.contains(categoriaSelecionada)) {
      item.style.display = 'block'; // Exibe o item se a categoria do item corresponder à categoria selecionada
    }
  });
}
...
function resetFiltro() {
  ...
  itens.forEach((item) => {
    item.style.display = 'block'; // Exibe todos os itens ao redefinir o filtro
  });
}
